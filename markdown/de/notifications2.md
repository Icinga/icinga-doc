...
einrichten (postfix: `/etc/postfix/main.cf`{.filename}, Direktive
"relayhost"; sendmail: `/etc/mail/sendmail.cf`{.filename}, Directive
...
-   Sie benötigen mindestens eine Kontaktdefinition ähnlich der
    ...
    -   Benachrichtigungen werden innerhalb des Zeitfensters "24x7" und
        ...
        Wir nehmen an, dass "24x7" 24 Stunden und sieben Tage die Woche
        und "workhours" Montag bis Freitag von 09:00 Uhr bis 17:00 Uhr
        ...
        beschriebenen Prüfungen erfolgreich durchlaufen wurden.
...
-   Sie benötigen verschiedene Befehlsdefinitionen für
    ...
    Bitte überprüfen Sie, dass die Programme (`printf`{.filename},
    `mailx`{.filename}) tatsächlich an den angegebenen Stellen
    existieren. Dies kann je nach Distribution abweichen.

    Es gibt viele
    [Makros](macrolist.md "5.3. Standard-Makros in Icinga"), die
    benutzt werden können, um die Benachrichtigungen mit Informationen
    zu füllen.
    ...
    Verschiedene Makros haben einen eingeschränkten Gültigkeitsbereich,
    z.B. sind CONTACT\*-Makros innerhalb von Event-Handlern nicht
    verfügbar.
...
-   Anstatt Meldungen zu versenden können Sie die Ausgabe auch in eine
    Datei umleiten, indem Sie "`| /usr/bin/mail         ...`{.literal}"
    durch etwas wie "`> /tmp/notification.log`{.literal}" ersetzen, um
    zu prüfen, ob alles Makros korrekt ersetzt werden

-   Prüfen Sie das Icinga-Log, Mail-Log und System-Log auf Meldungen

-   Editieren Sie `icinga.cfg`{.filename} und ändern Sie die folgenden
    Direktiven:
...
* * * * *
  ...
  ------------------------------- -------------------------- -------------------------------------
  [Zurück](notifications.md)    [Nach oben](ch05.md)      [Weiter](ch06.md)
  5.11. Benachrichtigungen        [Zum Anfang](index.md)    Kapitel 6. Die Benutzeroberflächen
  ------------------------------- -------------------------- -------------------------------------
