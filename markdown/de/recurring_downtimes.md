...
-   "downtime\_period" ist ähnlich zu der Definition, die in [time
    periods](objectdefinitions.md#objectdefinitions-timeperiod) zu
    finden ist. Das bedeutet, dass jede der folgenden Zeilen gültig sein
    sollte:
    ...
    01.01.2007 bis zum 01.02.2008 nicht eine einzelne, sondern für jeden
    Tag eine Ausfallzeit von 00:00 Uhr bis 24:00 Uhr geplant wird.
...
im Verzeichnis `/usr/local/icinga/etc`{.filename} nach
`holiday.cfg`{.filename} gesucht. Name und Standort können über die
...
-   Als eine aktive Prüfung
    ...
    Icinga-Server ausgeführt werden darf, nicht auf irgendeinem anderen
    in einer verteilten Umgebung.

-   Über die crontab

    Eine Zeile in der crontab des Icinga-Benutzers, das das Skript jede
    volle Stunde aufruft, könnte wie folgt aussehen
...
Die Perl-Skripte finden Sie im Ordner `tools/downtimes`{.filename}.

* * * * *

  ------------------------------- -------------------------- ------------------------------------------------
  [Zurück](downtime.md)         [Nach oben](ch07.md)      [Weiter](embeddedperl.md)
  7.16. Geplante Ausfallzeiten    [Zum Anfang](index.md)    7.18. Benutzen des Embedded Perl Interpreters
  ------------------------------- -------------------------- ------------------------------------------------
