...
-   [Monitoring Windows
    machines](monitoring-windows.md "2.13. Monitoring Windows Machines")

-   [Monitoring Netware
    servers](monitoring-netware.md "2.15. Monitoring Netware Servers")

-   [Monitoring Linux/Unix
    machines](monitoring-linux.md "2.14. Monitoring Linux/Unix Machines")
...
-   **Monitoring HTTP**
    ...
    point - either yours or someone else's. The *check\_http* plugin is
    ...
    *check\_http* plugin. It looks like this:
    ...
    *remotehost* machine might look like this:
    ...
    This simple service definition will monitor the HTTP service running
    on *remotehost*. It will produce alerts if the web server doesn't
    respond within 10 seconds or if it returns HTTP errors codes (403,
    404, etc.). That's all you need for basic monitoring. Pretty simple,
    huh?
    ...
    For more advanced monitoring, run the *check\_http* plugin manually
    with *--help* as a command-line argument to see all the options you
    can give the plugin. This *--help* syntax works with all of the
    plugins we'll cover in this document.
...
-   **Monitoring FTP**
    ...
    When you need to monitor FTP servers, you can use the *check\_ftp*
    plugin. The *commands.cfg* file contains a command definition for
    using the *check\_ftp* plugin, which looks like this:
    ...
    *remotehost* would look like this:
    ...
    generate an alert if the server doesn't respond within 5 seconds or
    if the server response doesn't contain the string "Pure-FTPd [TLS]".
...
-   **Monitoring SSH**
    ...
    When you need to monitor SSH servers, you can use the *check\_ssh*
    plugin. The *commands.cfg* file contains a command definition for
    using the *check\_ssh* plugin, which looks like this:
    ...
    *remotehost* would look like this:
    ...
    A more advanced service definition is shown below. This service will
    check the SSH server and generate an alert if the server doesn't
    respond within 5 seconds or if the server version string string
    doesn't match "OpenSSH\_4.2".
...
-   **Monitoring SMTP**
    ...
    The *check\_smtp* plugin can be using for monitoring your email
    servers. The *commands.cfg* file contains a command definition for
    using the *check\_smtp* plugin, which looks like this:
...
-   **Monitoring POP3**
    ...
    The *check\_pop* plugin can be using for monitoring the POP3 service
    on your email servers. The *commands.cfg* file contains a command
    definition for using the *check\_pop* plugin, which looks like this:
...
-   **Monitoring IMAP**
    ...
    The *check\_imap* plugin can be using for monitoring IMAP4 service
    on your email servers. The *commands.cfg* file contains a command
    definition for using the *check\_imap* plugin, which looks like
    this:
...
* * * * *
  ...
  2.17. Monitoring Routers and Switches    [Home](index.md)    Chapter 3. Configuring Icinga
  ---------------------------------------- -------------------- --------------------------------
